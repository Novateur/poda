...
function get_company_locations()
{
	...
	if($query->rowCount() > 0)
	{
		...
		foreach($query as $r)
		{
			echo "<p>".get_city_name($r['city'])." | ".get_state_name($r['state'])."<br/>{$r['addr']}<span style='float:right'><button type='button' onclick=\"delete_location({$r['id']})\"><i class='fa fa-trash'></i> Delete</button></span><p>";
		}
	}
}
