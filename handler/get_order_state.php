<?php
	...
	$id = sanitize($_POST['state']);
	
	$sql_pag = "SELECT COUNT(*) FROM orders WHERE printer_state='{$id}'";
	...
	$sql = "SELECT * FROM orders WHERE printer_state='{$id}' LIMIT $start,$limit";
							...
							echo "<div class='col s12'>";
								echo "<div align='center'id='paginate1'>";																
									...
									if($total_num_pages>1)
									{
										...
										if($previous>=1)
										{
											echo "<a href='#' onclick=\"paginate_state({$previous},{$id})\" data-role='button'>Previous</a> ";
										}
										for($i=1;$i<=$total_num_pages;$i++)
										{
											...
											{
												echo "<a href='#' onclick=\"paginate_state({$i},{$id})\" data-role='button'>{$i}</a> ";
											}
										}									
										if($next<=$total_num_pages)
										{
											echo "<a href='#' onclick=\"paginate_state({$next},{$id})\" data-role='button'>Next</a>";
										}
									}
								echo "</div>";
							echo "</div>";
