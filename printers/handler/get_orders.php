	...
	if($query->rowCount() > 0)
	{
		...
		foreach($query as $r)
		{
			            ...
			            echo "<tr>
							  ...
							  {
									echo "<td style='font-size:14px'>[ (".get_num_designs($r['itemid']).") <button type='button' onclick=\"get_designs_printer({$r['itemid']})\">View designs</button> ]"."</td>";
							  }
							  else
							  {
						  ...
						  if($r['progress']=="")
						  {
							echo "<button type='button' onclick=\"update_progress({$r['id']},{$page})\">Update progress</button></td>";
						  }
						  else
						  {
							echo "{$r['progress']} [ <button type='button' onclick=\"update_progress({$r['id']},{$page})\">Re-update</button> ]";
						  }
						  echo "</tr>";
		}
		echo "</tbody>";
